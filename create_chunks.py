...
class CSlamChunk:
    ...
    def save_scene(self, rgbimg, pts3d, msk, cams2world, focals, intrinsics):
        ...
        for i, img in enumerate(rgbimg):
            ...
            Image.fromarray(img).save(os.path.join(self.get_config_path(tag='images'), f"frame_{i}.png"))

            # Save point clouds (pts3d), masks (msk), and camera-to-world matrices (cams2world)
            np.save(os.path.join(self.get_config_path(tag='pts3d'), f"pts3d_{i}.npy"), pts3d[i])
            np.save(os.path.join(self.get_config_path(tag='msk'), f"msk_{i}.npy"), msk[i])
            np.save(os.path.join(self.get_config_path(tag='cams2world'), f"cams2world_{i}.npy"), cams2world[i])
            np.save(os.path.join(self.get_config_path(tag='focals'), f"focals_{i}.npy"), focals[i])
        # self._convert_scene_output_to_glb(os.path.join(self.get_config_path(tag='3d_models')), 
        #                                   rgbimg, pts3d, msk, focals, cams2world)
        # self._convert_scene_output_to_ply(os.path.join(self.get_config_path(tag='3d_models')), 
        #                                   rgbimg, pts3d, msk)
...
class CSlamPredictor:
    ...
    def generate_new_scene(self, file_list, save_dir, device='cuda', image_size=224, min_conf_thr=3, transparent_cams=True, cam_size=0.05):
        scene = self.get_reconstructed_scene(filelist=file_list, device=device, image_size=image_size, n_frame=len(file_list))
        scene_chunk = self.save_scene_info(scene, save_dir=save_dir, file_list=file_list, min_conf_thr=min_conf_thr, transparent_cams=transparent_cams, cam_size=cam_size)
        scene_chunk.parent_index = len(self.scenes) - 1
        self.scenes.append(scene)
        self.scenes_info.append(scene_chunk)

    def save_scene_info(self, scene, save_dir, file_list, min_conf_thr=3, transparent_cams=True, cam_size=0.05):
        ...
        current_chunk.save_scene(rgbimg=rgbimg, pts3d=pts3d, msk=msk, cams2world=cams2world, focals=focals, intrinsics=intrinsics)
...
if __name__ == '__main__':
    device = "cuda"
    model_name = "MVD"
    weights_path="/home/siyanhu/Gits/CSSlam/thirdparty/mvdust3r/checkpoints/MVD.pth"
    if model_name == "MVD":
    ...
    save_dir_general = '/media/siyanhu/T7/colmap_7s_camb/mvdust3r'
    fio.ensure_dir(save_dir_general)

    scene_name = 'scene_redkitchen'
    data_dir = f'/media/siyanhu/T7/colmap_7s_camb/sfm_7s/{scene_name}/train_full_byorder_85/images/'
    ...
    img_paths = sorted(img_paths, key=lambda path: int(os.path.basename(path).split('-')[1].split('.')[0]))
    ...
    for i in range(test_loop - 1):
        start_index = img_paths_subset_index[-1]
        img_paths_subset, img_paths_subset_index = select_random_chunk(img_paths)
        predictor.generate_new_scene(file_list=img_paths_subset, save_dir=save_dir, device=device)
